...
class Api_Common extends PhalApi_Api {
	...
	public function getUserGuard($uid, $liveuid) {
		$info=$this->getCache("usergurad_".$uid."_".$liveuid);
		if(!$info){
			$domain = new Domain_Common();
			$info = $domain->getUserGuard($uid, $liveuid);				
		}
	...
	public function sendCode($mobile,$code){
		...
		require_once "../../simplewind/Lib/Extend/sms.php";
		...
		$data = array(
			'content' 	=> '【房一起】验证码：'.$code.'。请不要把验证码泄露给其他人。如非本人操作，可不用理会',//短信内容
			'mobile' 	=> $mobile,//手机号码
			'productid' => '676767',//产品id
			'xh'		=> ''//小号
		);
		$sendAPI->data = $data;//初始化数据包
		$sendAPI->sendSMS('POST');//GET or POST 
        ...
        $gets = array('SubmitResult'=>array('code'=>2,'msg'=>'成功'));
