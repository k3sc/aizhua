...
//    //点击发送信息设置
    ...
    flymsgfn:function(url){
        if(fly=="FlyMsg"){
			...
			var msg = '{"msg":[{"_method_":"SendMsg","action":0,"ct":"'+$("#message").val()+'","msgtype":"2","uid":"'+User.id+'","uname":"'+User.user_nicename+'","level":"'+User.level+'"}],"retcode":"000000","retmsg":"OK"}';
		...
		$.ajax({
            ...
            success:function(data){
				...
				if(data.errno==0){
					...
					var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendGift","evensend":"'+data.evensend+'","action":"0","ct":"'+data.gifttoken+'","msgtype":"1","level":"'+data.level+'","uid":"'+data.uid+'","uname":"'+User.user_nicename+'","uhead":"'+User.avatar+'"}]}'; 
					Socket.emitData('broadcast',msg);	
				}else{
					_this.countdown_hide();
