...
class Wxpaysdk
{
	...
	public function getAppPackAge($prepayid) {
		$data = array();
		$data["appid"] = $this->appid;
		$data["partnerid"] = $this->mchid;
		$data["prepayid"] = $prepayid;
		$data["package"]   = "Sign=WXPay";
		$data["noncestr"]  = $this->createNoncestr(8);
		$timeStamp = time();
	 ...
	 * 	作用：设置jsapi的参数
	 */
	public function getJSAPI($prepayId) {
		...
		$wOpt['package'] = 'prepay_id=' . $prepayId;//$this->prepay_id;
		$wOpt['signType'] = 'MD5';
