...
var liveType=
{
	...
	{
		 // liveType.selectPlay(); 
	 	var choice='{"title":"未设置标题","type":"","stand":""}';
   ...
   $.ajax({
	...
	other:function()
	{
		...
		liveType.closePorp();
		var choice='{"title":"未设置标题","type":"'+type+'","stand":"'+stand+'"}';
		document.getElementById('webplayer')._myclock(choice);
	},
		...
		if(_DATA.live && _DATA.live.islive==1)
		{
			...
			$.ajax({
				...
				success:function(data){
					if(data.land==0)
					{
						if(data.type>0 && data.type_msg!="")
						{
							if(data.type==1){
							var mag_name="当前房间为密码房间";
							liveType.passRoom(data.type_msg,mag_name);
							}else if(data.type==2){
							liveType.charge(data.type_msg,liveuid,stream);
							}else if(data.type==3){
	...
	charge:function(type_msg,liveuid,stream)
	{
		var t=setTimeout("window.location.href='/'",20000)
		layer.confirm(type_msg, {
			btn: ['进入','放弃'] //按钮
		}, function(){
			$.getJSON("./index.php?g=home&m=Spend&a=roomCharge", {liveuid:liveuid,stream:stream},
			function(data)
      {  
				if(data.code!=0)
				{
 		...
 		$.ajax({
			...
			success:function(data){
				if(data.code==0){
					...
					var msg = '{"msg":[{"_method_":"ShutUpUser","action":"1","ct":"'+name+'被禁言了5分钟","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nicename+'","touid":'+touid+',"toname":"'+name+'"}],"retcode":"000000","retmsg":"ok"}';
					Socket.emitData('broadcast',msg);
				}else{
					layer.msg(data.msg);
				}
		...
		$.ajax({
			...
			success:function(data){
				if(data.ret==200 && data.data.code==0){
					 var msg = '{"msg":[{"_method_":"stopLive","action":"10","ct":"该直播间涉嫌违规，已被停播","msgtype":"4","uid":'+_DATA.user.id+',"uname":"'+_DATA.user.user_nicename+'","touid":'+_DATA.anchor.id+',"touname":"'+_DATA.anchor.id.user_nicename+'"}],"retcode":"000000","retmsg":"ok"} ';
					Socket.emitData('broadcast',msg);
		...
		if(coin<needcoin &&_DATA.user.coin<needcoin){
			if(!Check.checkCoin(needcoin)){
				(new Dialog).tip('<div class="status-no-money" id="_temp_DDS_noEnoughMoney">\
									...
									<div class="opt">\
										<a class="BTN BTN-recharge" href="./index.php?m=Payment&a=index" target="_blank">充值</a>\
									</div>\
			...
			success:function(data){
				...
				if(data.code!=0){
				  layer.msg(data.msg);
					...
					_DATA.user.coin=data.coin;
					User.updateMoney();
					var msg='{"msg":[{"sex":"","action":"7","city":"","usign":"","ugood":"'+_DATA.user.id+'","roomnum":"'+_DATA.anchor.id+'","level":"'+data.level+'","timestamp":"","equipment":"app","uid":"'+_DATA.user.id+'","ct":"'+data.barragetoken+'","touid":"0","_method_":"SendBarrage","msgtype":"1","tougood":"","uhead":"'+_DATA.user.avatar+'","uname":"'+_DATA.user.user_nicename+'","touname":""}],"retmsg":"OK","retcode":"000000"}';
					 Socket.emitData('broadcast',msg);
      ...
      type: "POST",
			success:function(data){
				...
				if(data.errno!=0){
					...
					var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendGift","evensend":"'+data.evensend+'","action":"0","ct":"'+data.gifttoken+'","msgtype":"1","level":"'+data.level+'","uid":"'+data.uid+'","timestamp":"'+WlTools.FormatNowDate()+'","uname":"'+_DATA.user.user_nicename+'","uhead":"'+_DATA.user.avatar+'"}]}'; 
					Socket.emitData('broadcast',msg);
				}
			},
			error: function(data)
...
var Light={
	...
	light:function()
	{
		...
		var light = '{"msg":[{"_method_":"light","action":"2","msgtype":"0"}],"retcode":"000000","retmsg":"OK"}';
