...
<div class="wrap">
    ...
    <form method="post" class="well form-search" action="<?php echo U('Activeconfig/index');?>">
        <span>启用时间：</span>
        ...
        <select name="status" style="width:auto;margin-top:5px;">
            ...
            <option value="1" <?php if($_POST['status']== '1'): ?>selected="selected"<?php endif; ?>>已启用</option>
            <option value="2" <?php if($_POST['status']== '2'): ?>selected="selected"<?php endif; ?>>未启用</option>
        </select>
    ...
    <form method="post" class="js-ajax-form" action="<?php echo U('Ads/listorders');?>">
        <table class="table table-hover table-bordered">
            ...
            <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
                    ...
                    <td>
                        <?php if($vo['atime'] != 0): echo (date("Y-m-d H:i:s",$vo['atime'])); ?></td>
                        <?php else: ?>
                            无<?php endif; ?>
                    </td>
                    <td><?php echo (date("Y-m-d H:i:s",$vo['ctime'])); ?></td>
                    <td style="text-align:center;">
                        <a href="<?php echo U('Activeconfig/del',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" data-msg="您确定要删除吗？">删除</a>
                        <?php if($vo['status'] != 1): ?>| <a href="<?php echo U('Activeconfig/action',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" data-msg="启用上币活动需要一定时间，请耐心等待系统执行完毕...">启用</a>
                            | <a href="<?php echo U('Activeconfig/add_act',array('id'=>$vo['id']));?>" >编辑</a><?php endif; ?>
                    </td>
