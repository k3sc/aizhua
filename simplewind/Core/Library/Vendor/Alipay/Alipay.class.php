...
class Alipay
{
    ...
    function __construct($config, $is_mobile = FALSE)
    {
        ...
        if ($is_mobile) {
            ...
            $this->alipay_gateway_new = $this->alipay_gateway_mobile;
        }
        if($this->config['service']){
            $this->service = $this->config['service'];
            $this->service_wap = $this->config['service'];
        }
    }
...
//		return $html;
    ...
    function buildRequestFormString($params)
    {
        $params = $this->buildSignedParameters($params);
        $url = '';
    ...
    {
        $fp = fopen("./Data/alipay_notify.log", "a");
        flock($fp, LOCK_EX);
