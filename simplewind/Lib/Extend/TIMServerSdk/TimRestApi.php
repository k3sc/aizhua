...
class TimRestAPI extends TimRestInterface
{
	...
	protected $im_yun_url = '183.61.56.115';//'console.tim.qq.com';
	...
	//不需要结果
	public function multi_api_null($service_name, $cmd_name, $identifier, $usersig, $req_data, $print_flag = true)
	{   
		...
		$this->http_req_multi_null('https', 'post', $url, $req_data);
	}
	...
	//不需要结果
	public static function http_req_multi_null($http_type, $method, $url, $data)
	{
		$mh = curl_multi_init();
	...
	public function querystate($accounts = array()){
		$msg = array('To_Account' => $accounts);
		$req_data = json_encode($msg);
		$ret = $this->api("openim", "querystate", $this->identifier, $this->usersig, $req_data, false);
        $ret = json_decode($ret, true);
        echo json_format($ret);
		return $ret;
	}
