...
class AdminbaseController extends AppframeController {
    ...
    protected function uploadImage($file){
        ...
        if ($info = $upload->upload($file)) {			
            $str = '';
            foreach ($info as $k => $v) {
				if(!$v['url'])$v['url'] = '/data/upload/'.$v['savepath'].$v['savename'];
                $str = $str.$v['url'].',';
            }
     ...
     * @throws \Exception
     */
    protected function umengpush_listcast($androidtoken, $iphonetoken, $title, $message,$mesid)
    {
        require_once('./simplewind/Lib/Extend/umeng_php_sdk_v1.5/php/src/notification/android/AndroidListcast.php');
        require_once('./simplewind/Lib/Extend/umeng_php_sdk_v1.5/php/src/notification/ios/IOSListcast.php');
//        $id = '';
//        if ($androidtoken) {
//            $id = M('users')->where(['androidtoken' => $androidtoken])->getField('id');
//        } else if ($iphonetoken) {
        ...
        if (!empty($androidtoken)) {
            try {
               ...
               $unicast->setAppMasterSecret('yftyxfo9mce09rhwt69l3t5uurku1kqc');
                ...
                // Set extra fields
//                $unicast->setExtraField("user_id", $id);
                $unicast->setExtraField("mesid", $mesid);
                //$unicast->setExtraField("order_id", $order_id);
                $unicast->send();
            } catch (Exception $e) {
                //
        ...
        if (!empty($iphonetoken)) {
            try {
                ...
                // Set customized fields
//                $unicast->setCustomizedField("user_id", $id);
                $unicast->setCustomizedField("mesid", $mesid);
                //$unicast->setCustomizedField("order_id", $order_id);
                $unicast->send();
            } catch (Exception $e) {
                //
     ...
     * @param $iphonetoken
     * @param $title
     * @param $type 0安卓  1 ios
     * @param $message
     * @throws \Exception
     */
    protected function umengpush_broadcast($title, $message,$mesid,$type=0)
    {
        require_once('./simplewind/Lib/Extend/umeng_php_sdk_v1.5/php/src/notification/android/AndroidBroadcast.php');
        require_once('./simplewind/Lib/Extend/umeng_php_sdk_v1.5/php/src/notification/ios/IOSBroadcast.php');
 		if(!$type){
                $brocast = new \AndroidBroadcast();
                $brocast->setAppMasterSecret('yftyxfo9mce09rhwt69l3t5uurku1kqc');
                $brocast->setPredefinedKeyValue("appkey", '5a1e663ef29d986cf9000160');
                $brocast->setPredefinedKeyValue("timestamp", time());
                $brocast->setPredefinedKeyValue("ticker", $title);
                $brocast->setPredefinedKeyValue("title", $title);
                $brocast->setPredefinedKeyValue("text", $message);
                $brocast->setPredefinedKeyValue("after_open", "go_custom");
                $brocast->setPredefinedKeyValue("custom", "go_custom");
                $brocast->setPredefinedKeyValue('icon', 'small_app_logo');
                $brocast->setPredefinedKeyValue('largeIcon', 'app_logo');
                $brocast->setPredefinedKeyValue("production_mode", "true");
                // Set extra fields
                $brocast->setExtraField("mesid", $mesid);
                $brocast->send();
		}else{
                $unicast = new \IOSBroadcast();
                $unicast->setAppMasterSecret('1awu0wjgjxv5w0wfupbpjgivnbwpufrl');
                $unicast->setPredefinedKeyValue("appkey", '5a1e45a08f4a9d570c000142');
                $unicast->setPredefinedKeyValue("timestamp", time());
                // Set your device tokens here
