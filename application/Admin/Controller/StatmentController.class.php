...
*/
    ...
    public function index()
    {
        ...
        if ($get_time[2] == 'day' || $get_time[2] == 'week') {
    	    ...
    	    $cb = M('waybill')->alias('a')->field('sum(b.cost) as total ')
                ->join('cmf_give_gift as b on a.user_gift_id = b.id')
                ->where('a.fhtime >= ' . $get_time[0] . ' and a.fhtime <= ' . $get_time[1] . ' and a.status >= 2 and user_gift_id != ""')
                ->find();
            $result[$d]['cb_totals'] = $cb['total'];
	$cb = M('waybill')->alias('a')->field('sum(c.cost) as total')
                ->join('cmf_user_wawas as b on a.user_wawas_id = b.id')
                ->join('cmf_gift as c on b.wawa_id = c.id')
                ->where('a.fhtime >= ' . $get_time[0] . ' and a.fhtime <= ' . $get_time[1] . ' and a.status >= 2 and user_wawas_id != "" ')
                ->find();
            if (!isset($result[$d]['cb_totals']))
                $result[$d]['cb_totals'] = 0;
            $result[$d]['cb_totals'] = $result[$d]['cb_totals']+$cb['total'];
		//echo M('waybill')->_sql();exit;
	   ...
	   $cb = M('waybill')->alias('a')->field('sum(b.cost) as total, left(FROM_UNIXTIME(a.fhtime),' . $sTime . ') as time')
                ->join('cmf_give_gift as b on a.user_gift_id = b.id')
                ->where('a.fhtime >= ' . $get_time[0] . ' and a.fhtime <= ' . $get_time[1] . ' and a.status >= 2 and user_wawas_id != ""')
                ->group('time')->select();
//            echo M('waybill')->_sql();
//            print_r($cb);exit;
            foreach ($cb as $k => $v) {
                $result[$v['time']]['cb_totals'] = $v['total'];
            }
            $cb = M('waybill')->alias('a')->field('sum(c.cost) as total, left(FROM_UNIXTIME(a.fhtime),' . $sTime . ') as time')
                ->join('cmf_user_wawas as b on a.user_wawas_id = b.id')
                ->join('cmf_gift as c on b.wawa_id = c.id')
		->where('a.fhtime >= ' . $get_time[0] . ' and a.fhtime <= ' . $get_time[1] . ' and a.status >= 2 and user_wawas_id != "" ')
                ->group('time')->select();
            foreach ($cb as $k => $v) {
                if (!isset($result[$v['time']]['cb_totals']))
                    $result[$v['time']]['cb_totals'] = 0;
                $result[$v['time']]['cb_totals'] = $result[$v['time']]['cb_totals']+$v['total'];
            }
        }
...
//        $device_uni = I('device_uni','');
        ...
        $gArr = M('game_history')->alias('a')
            ...
            ->join('cmf_game_room as b on a.`room_id` = b.`id`', 'inner')
            ->join('cmf_device AS c ON b.device_id = c.`id`', 'left')
            ->join('cmf_user_wawas AS d ON a.`success` = d.`id`', 'left')
            ->join('cmf_gift AS e ON b.`type_id` = e.`id`', 'left')
            ->join('cmf_device_addr as f on c.device_addr_id = f.id')
            ->where('a.ctime >= ' . $get_time[0] . ' and a.ctime <= ' . $get_time[1])
            ->group('a.`room_id`')
            ->select();
