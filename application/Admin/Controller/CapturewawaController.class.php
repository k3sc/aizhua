...
//            $this->assign('keyword',$keyword);
    ...
    public function edit_post()
    {
	    ...
	    $row = M('gift')->where(['id' => $post['id']])->find();

            $old = $row['convert_lipin'];//echo $old."<br>";
            $new = isset($post['convert_lipin']) ?  $post['convert_lipin'] : '';
	    ...
	    if ($del)
            {
                ...
                $result = M('give_gift')->where('id in('.implode(',',$del).')')->select();
                foreach ($result as $row)
                {
                    $ids = explode(',',$row['body_id']);
                    $ids = array_diff($ids,[$post['id']]);
                    $sql ='update cmf_give_gift set body_id = "'.implode(',',array_unique($ids)).'" where id = '.$row['id'];
                    M()->execute($sql);
                }
            ...
            if ($add)
            {
                ...
                $result = M('give_gift')->where('id in('.implode(',',$add).')')->select();
                foreach ($result as $row)
                {
                    $ids = explode(',',$row['body_id']);
            ...
            if ($post['id']) {
	    	if( $post['convert_lipin'] ){
			/* 关联礼品 */
	    		//$post['id'] .= ',';
			//$sql = "update cmf_give_gift set body_id = CONCAT('".$post[id]."',body_id) where id in ($post[convert_lipin])";
			//M()->execute($sql);
		}
    ...
    public function add_post()
    {
        if (IS_POST) {
            ...
            unset($post['file']);
            $post['addtime'] = time();
            if ($res = M('gift')->add($post)){
		    /* 关联礼品 */
		    $res .= ',';
		    $sql = "update cmf_give_gift set body_id = CONCAT('".$res."',body_id) where id in ($post[convert_lipin])";
		    M()->execute($sql);
