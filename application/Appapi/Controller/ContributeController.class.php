...
class ContributeController extends HomebaseController {
	...
	public function order(){
		...
		if($type=='day'){
			...
			$list=M("users_coinrecord")->field("uid,sum(totalcoin) as total")->where(" action in ('sendgift','sendbarrage') and touid='{$uid}' and addtime>{$today_start} and addtime<{$today_end}")->group("uid")->order("total desc")->limit(0,20)->select();
			...
			$list=M("users_coinrecord")->field("uid,sum(totalcoin) as total")->where(" action in ('sendgift','sendbarrage') and touid='{$uid}' and addtime>{$month_start} and addtime<{$month_end}")->group("uid")->order("total desc")->limit(0,20)->select();
