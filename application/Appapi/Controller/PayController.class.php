...
class PayController extends HomebaseController {
	...
	public function notify_ali() {
		...
		require_once("./alipay_app/alipay.config.php");
		require_once("./alipay_app/lib/alipay_core.function.php");
		require_once("./alipay_app/lib/alipay_rsa.function.php");
		require_once("./alipay_app/lib/alipay_notify.class.php");
		...
		if($verify_result) {//验证成功
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
				...
				//调试用，写文本函数记录程序运行情况是否正常
				//logResult("这里写入想要调试的代码变量值，或其他运行的结果记录");
				$this->logali("_POST:".json_encode($_POST));	
				$orderinfo=M("users_charge")->where("orderno='{$out_trade_no}'  and status='0' and type='1'")->find();	//and money='{$total_fee}'
	...
	public function notify_wx(){
		...
		if($arrayInfo['return_code'] == "SUCCESS"){
			...
			echo $this -> returnInfo("FAIL","签名失败");
			$this -> logwx('签名失败:'.$arrayInfo['return_code']);//log打印保存
			exit;
		}			
	}
