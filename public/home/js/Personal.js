...
var Personal={
      ...
      if(radios[i].checked==true)
       {
		...
		$.ajax({
			cache: true,
			type: "POST",
			url:'/index.php?m=Personal&a=edit_modify&nickName='+nickName+"&signature="+signature+"&sex="+sex+"&birthday="+birthday,
			async: false,
	...
	admin_del:function(touid)
	{
		$.ajax({
			cache: true,
			type: "POST",
			url:'/index.php?m=Personal&a=admin_del&touid='+touid,
			async: false,
			error: function(request)
			{
				layer.msg("数据请求失败");
	...
	blacklist:function(touid)
	{
		$.ajax({
			cache: true,
			type: "POST",
			url:'/index.php?m=Personal&a=blacklist&touid='+touid,
			async: false,
			error: function(request)
			{
				layer.msg("数据请求失败");
	...
	{
		$.getJSON("./index.php/Personal/follow_dal/followID/"+followID+"/", {},
    function(data) 
		{
			if(data["state"]==0)
			{
	...
	{
		$.getJSON("./index.php/Personal/follow_add/",{touid:touid},
    function(data) 
		{
			if(data["state"]==0)
			{
    ...
    $.ajax({
      type:"post",
      url:"index.php/Playback/index/touid/"+touid+"/",
      data:"",
      success:function(data)
			{
        if(data=='2')
	...
	{
		$.getJSON("./index.php/Personal/list_del/",{touid:touid},
    function(data) 
		{
			if(data["state"]==0)
			{
	...
	exchange:function()
	{
		$.getJSON("./index.php/Personal/edit_exchange/", {},
    function(data) 
		{
			console.log(data);
			 if(data['state']==0)
	...
	photo:function()
	{
		...
		$.getJSON("./index.php?m=Personal&a=edit_photo", {avatar:avatar},
    function(data) 
		{
			if(data['error']==1)
			{
	...
	updatepass:function()
	{
		...
		var repass=$("#repass").val();
			 if(newpass!=""&&repass!="")
			{
				$.getJSON("./index.php/Personal/savepass/oldpass/"+oldpass+"/newpass/"+newpass+"/repass/"+repass, {},
				function(data) 
				{
					if(data["code"]==0)
					{
